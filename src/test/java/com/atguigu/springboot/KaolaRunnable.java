 ...
 * desc：
 */
public class KaolaRunnable implements  Runnable {

    private KlCategory category;
    public KaolaRunnable(){
    }
    public KaolaRunnable(KlCategory category){
    ...
    public void run() {
        ...
        try {
            List<KlGoods> goodsList=getGoods(rrUrl);
    ...
    public static void main(String[] args) {
        String categoryUrl="https://search.kaola.com/category/1471.html";
        List<KlGoods> datalist= null;
        KaolaRunnable dd= new KaolaRunnable();
        try {
            datalist =dd.getGoods(categoryUrl);
            KlCategory category=new KlCategory() ;
            category.setCategoryName("面膜");
            category.setParentName("美容彩妆");
            category.setGoodsList(datalist);
            dd.createExcel(category);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public  void  createExcel(KlCategory category) throws  Exception{
        ...
        buildExcelFile("网易考拉/"+category.getParentName()+"/"+sheetName+".xls",workbook);
